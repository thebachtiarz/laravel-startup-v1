...
const sidebarLinkCall = () => {
    axios.get(`/apps/helper/global/allowed-link`, { params: { _type: 'sidebar' }, timeout: 5000 }).then(response => { if (response.data['message'] == 'no-auth') { panelLogin() } sidebarLinkBar(response.data['postscript'][0]) }).catch(error => toastError(error));
}
const sidebarLinkBar = (data) => {
    ...
    data.forEach(sidebar => { link += `<li class="nav-item"><a href="${sidebar.link}" class="nav-link userLink${(Request(1) == sidebar.link.substr(1) || (!Request(1) && sidebar.link == '/')) ? ' active' : ''}"><i class="${sidebar.icon}"></i>&ensp;<p class="text-sm">${sidebar.name}</p></a></li>` });
    link += `</ul>`;
