...
const responseLogin = async (data) => {
    await sleep(1000);
    (data.status == 'success') ? ($('#login-head-message').html(`<label class="text-success">Welcome ${data.response_data[0].name}</label>`), localStorage.setItem('_jwtApiToken', data.response_data[0].token), redirectTo('/api/home')) : $('#login-head-message').html(`<label class="text-info">${data.message}</label>`);
}
