...
const responseLogin = async (data) => {
    await sleep(1000);
    (data.status == 'success') ? ($('#login-head-message').html(`<label class="text-success">Welcome ${data.response_data[0].name}</label>`), localStorage.clear(), localStorage.setItem('_jwtApiToken', data.response_data[0].token), localStorage.setItem('_credentials', JSON.stringify(data.response_data[0])), redirectTo('/home')) : $('#login-head-message').html(`<label class="text-info">${data.message}</label>`);
}

$(createLoginForm());
$(async () => { await sleep(2000), checkAuth() });
