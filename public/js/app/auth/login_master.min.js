const checkAuth = () => {
    ...
    if (token) {
        let user = getUserLocalStorage() ? getUserLocalStorage() : postUser();
        if (user) {
            $('#login-head-message').html(`<label class="text-success">Welcome Back ${user.name}</label>`), redirectTo('/api/home')
        } else {
...
$(document).on('click', '#submitLogin', function () {
    ...
    if (email && password) { submitLogin(email, password), $('#email-form').removeClass('mb-1'), $('#email-form').addClass('mb-3'), $('#email-warn').hide(), $('#password-form').removeClass('mb-1'), $('#password-form').addClass('mb-3'), $('#password-warn').hide() }
...
const responseLogin = async (data) => {
    await sleep(1000);
    (data.status == 'success') ? ($('#login-head-message').html(`<label class="text-success">Welcome ${data.response_data[0].name}</label>`), localStorage.clear(), localStorage.setItem('_jwtApiToken', data.response_data[0].token), localStorage.setItem('_credentials', JSON.stringify({ 'name': data.response_data[0].name, 'email': data.response_data[0].email })), redirectTo('/api/home')) : $('#login-head-message').html(`<label class="text-info">${data.message}</label>`);
}
